...
// "kuiper_full_64_image" -> {"full", "arm64"}. The build encodes the ARM word
// size as "32"/"64"; we surface it as arm32/arm64. Falls back to the raw name
// (minus "_image") with an empty arch if the shape is unexpected.
...
Result<std::vector<Release>> GitHubActionsProvider::list(
    ...
    // One request lists the recent runs; then one request per run for its
    // artifacts (1 + N). N is small (limit is clamped to 100), but note this
    // spends the unauthenticated rate budget quickly — hence the optional token.
