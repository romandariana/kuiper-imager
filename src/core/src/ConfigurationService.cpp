...
// Build a Project from one `.projects[]` element, mirroring the script's
// filter: `select(has("name") and has("board"))`. Returns nullopt for entries
// that aren't objects or lack a non-empty name/board.
...
// Parse a single manifest file, appending any projects it declares. Silently
// tolerates unreadable / malformed / non-manifest JSON (the script's jq
// pipeline does the same via `2>/dev/null` and `.projects[]?`).
...
// Resolve a manifest boot-file path to its on-card source. The manifests store
// absolute paths rooted at `/boot`; the mounted partition may live elsewhere
// (e.g. /run/media/$USER/BOOT). Mirrors the script's `sed s|^/boot|$BOOT|`.
...
Result<ConfigureSummary> ConfigurationService::configureProject(
    ...
    if (isIntel) {
        ...
        // Mirror the script's `mkdir -p extlinux && mv extlinux.conf extlinux/`.
    ...
    // Non-intel boards must not carry a stale extlinux/ from a prior intel
    // config; the script removes it. Best-effort — absence is fine.
