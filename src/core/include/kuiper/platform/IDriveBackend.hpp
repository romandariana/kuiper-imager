...
public:
    ...
    // Mount a partition and hand back an RAII handle to its filesystem path. If
    // the partition is already mounted, the handle is borrowed (its mountpoint,
    // left untouched on destruction); otherwise the backend mounts it to a
    // private temp dir and the handle unmounts + cleans up on destruction.
    // Errors if the partition has no filesystem to mount.
    virtual Result<MountedPartition> mount(const Partition& partition) = 0;

    // --- Raw device I/O (used by DriveService::flash) ------------------------
    //
    // These operate on a single device opened by openForWrite/openForRead. The
    // funnel invariant lives here: only the platform backend touches syscalls,
    // ioctls, and device nodes. Short-write/short-read/EINTR loops are handled
    // internally so write() is all-or-error and read() fills the buffer.
    //
    // Block alignment (O_DIRECT) is entirely the backend's concern: callers pass
    // arbitrary offsets/lengths and the backend stages/bounces to satisfy the
    // hardware. Offsets passed to seek() must be block-aligned; the flash caller
    // only ever seeks to block-aligned positions (0 and the deferred-head size).
    ...
    //
    // Read-only lookups over the block tree. Like listDrives, they shell out to
    // util-linux (lsblk); the funnel invariant keeps that here.
