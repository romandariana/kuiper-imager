...
// A configurable hardware project, parsed from a BOOT-partition manifest
// (*.json). Mirrors the fields the ADI configure-setup.sh script reads from
// `.projects[]`. `(name, board)` identifies a project; the same `name` (eval
// board) recurs across carriers.
struct Project {
