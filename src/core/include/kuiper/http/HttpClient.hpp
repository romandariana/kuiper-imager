...
// responses for list-releases); `download()` streams an arbitrarily large body
// to a sink without buffering it (fetch of multi-GB images). Injected into the
// service and its providers so they are unit-testable with a fake, exactly like
// IDriveBackend.
class IHttpClient {
public:
